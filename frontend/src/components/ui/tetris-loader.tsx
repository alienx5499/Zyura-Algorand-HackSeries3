...
// Tetris pieces - pure black and white (inverted)
const TETRIS_PIECES = [
  // I-piece
...
export default function TetrisLoading({
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map((cell) => ({
              ...cell,
              color: "bg-white dark:bg-white animate-pulse opacity-50",
            }));
          }
          return row;
        });
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-gray-600 dark:border-gray-300 transition-all duration-100 ${
              cell.filled
                ? `${cell.color} scale-100`
                : "bg-black dark:bg-black scale-95"
  ...
  return (
    <div>
      <div>
        <div
          className={`border-2 border-gray-200 dark:border-gray-800 bg-black dark:bg-black ${config.padding} transition-colors`}
        >
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-white dark:text-white text-sm font-light transition-colors">
            {loadingText}
          </p>
        </div>
      )}
